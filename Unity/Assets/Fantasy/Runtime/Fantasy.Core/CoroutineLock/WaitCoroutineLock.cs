...
namespace Fantasy.Core
{
    ...
    public sealed class OnCoroutineLockTimeout : EventSystem<CoroutineLockTimeout>
    {
        ...
        public override void Handler(CoroutineLockTimeout self)
        {
            ...
            FLog.Error($"coroutine lock timeout CoroutineLockQueueType:{coroutineLockQueueType.Name} Key:{key} Tag:{self.WaitCoroutineLock.Tag}");
        }
    }
    ...
    public sealed class WaitCoroutineLock : IDisposable
    {
        ...
        public void Dispose()
        {
            if (IsDisposed)
            {
                FLog.Error("WaitCoroutineLock is Dispose");
                return;
            }
